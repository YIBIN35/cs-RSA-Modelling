...
df_phrase['phrase'] = df_phrase.apply(
    lambda r: f"{r['adj_utterance']} {r['noun']}",
    axis=1
)
...
df_combined = df_combined[['noun','utterance','image', 'state', 'adj', 'adj_utterance']].reset_index(drop=True)
...
for random_number in [1]:
    ...
    assert set(map(tuple, df_shuffled.to_numpy())) == set(map(tuple, df_combined.to_numpy()))
